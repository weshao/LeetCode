    ...
    def replaceWords(self, dict, sentence):
        ...
        for prefix in dict:
            for index, word in enumerate(words):
                if word[:len(prefix)] == prefix:            # 如果这个单词以prefix为前缀
                    words[index] = prefix               # 就把这个单词替换为prefix
        return ' '.join(words)
# @lc code=end

so = Solution()
ans = so.replaceWords( ["a", "aa", "aaa", "aaaa"], "a aa a aaaa aaa aaa aaa aaaaaa bbb baba ababa")
print(ans)