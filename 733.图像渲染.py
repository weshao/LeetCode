...
class Solution:
    def floodFill(self, image: List[List[int]], sr: int, sc: int, newColor: int) -> List[List[int]]:
        ...
        if oldColor == newColor:
            return image
        self.recur(image, sr, sc, newColor, oldColor)
        return image

    def recur(self, image, sr, sc, newColor, oldColor):

        image[sr][sc] = newColor
        if sr > 0 and oldColor == image[sr-1][sc]:
            self.recur(image, sr - 1, sc, newColor, oldColor)
        if sr < len(image) - 1 and oldColor == image[sr+1][sc]:
            self.recur(image, sr + 1, sc, newColor, oldColor)
        if sc < len(image[0]) - 1 and oldColor == image[sr][sc+1]:
            self.recur(image, sr, sc + 1, newColor, oldColor) 
...
# @lc code=end
so = Solution()
ans = so.floodFill([[0, 0, 0], [0, 1, 1]], 1, 1, 1)
print(ans)
