...
class Solution:
    def calcEquation(self, equations: List[List[str]], values: List[float], queries: List[List[str]]) -> List[float]:
        ...
        for i in range(len(equations)):
            parent, child = equations[i][0], equations[i][1]
            graph[parent].append((child, values[i]))
            indegree[child] += 1
        ...
        while queue:
            ...
            for _ in range(n):
                ...
                for child, value in graph[parent]:
                    nums[child] = nums[parent] / value
                    queue.append(child)
        ...
        for num1, num2 in queries:
            if num1 not in nums or num2 not in nums:
                ...
                res.append(round(nums[num1] / nums[num2], 5))
        
        return res
            
# @lc code=end
so = Solution()
ans = so.calcEquation(equations = [["a","b"]], values = [0.5], queries = [["a","b"],["b","a"],["a","c"],["x","y"]])
print(ans)
