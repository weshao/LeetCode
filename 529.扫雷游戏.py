...
class Solution:
    def updateBoard(self, board: List[List[str]], click: List[int]) -> List[List[str]]:
      ...
      if not nr:
        return board
      nc = len(board[0])
      self.dfs(board, click[0], click[1], nr, nc)
      return board
    
    def dfs(self, board, i, j, nr, nc):
      count = 0
      if board[i][j] == 'M':
        board[i][j] = 'X'
      else:
        ...
        if not count:
          board[i][j] = 'B'
          for row, col in [(i-1, j-1), (i-1, j), (i-1, j+1), (i, j-1), (i, j+1), (i+1, j-1), (i+1, j), (i+1, j+1)]:
            if 0<=row<nr and 0<=col<nc and board[row][col] == 'E':
              self.dfs(board, row, col, nr, nc)
        else:
          board[i][j] = str(count)
